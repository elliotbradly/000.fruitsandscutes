//creates template html page 
//adds this element to the library of html files
...
list.forEach((a, b) => {
  ...
  if (items[0] != pivot ) return
   pivot = items.join('.')
});

console.log("show me pivot " + pivot )

var srcDir = "./" + pivot + "/see/";
var finFle = "./" + pivot + "/val/html.ts";

FS.ensureDirSync(srcDir)
FS.ensureFileSync( finFle )

var dex = FS.readdirSync( srcDir).length;
dex = String(dex).padStart(3, "0");

var output = srcDir + dex + '.' + title + '.html';
var template = '<div class="divider text-center">' + title + '</div>';
...
var glop = 'export const {{=it.title}}: string = require("../../../{{=it.file}}");'

list.forEach( (a)=>{
  console.log( "a " + a)
  var want = a.split('.'); 
  if ( want.length != 3) return
  var title = S(want[1] ).camelize().s;
  var file =  pivot + "/see/" + a;

  var gel = {title, file}
  
  var doTCompiled = doT.template(glop);
  var outLine = doTCompiled(gel);
  line.push( outLine)
})

var final = line.join('\n')
FS.writeFileSync( finFle, final)
